# Naveg-AI - Compiled Standalone Script
...
SBSJ_RNP_34_DAGOV_ROUTE = [
    ...
    {"name": "RWY34", "lat": -23.236727, "lon": -45.853452, "alt_ft": 2571, "desc": "MAPT (Pista 34)"}
]
...
class PFDGui:
    def __init__(self, base_size=(1800, 900)):
        ...
        self.f_label = pygame.font.SysFont("Segoe UI", 16)
        ...
        # Linha 2 - Lado Esquerdo: Entrada do Curso Manual
        self.course_input_rect = pygame.Rect(600, 175, 180, 46)
        self.btn_set_course = pygame.Rect(800, 175, 110, 46)

        # Linha 2 - Lado Direito: Tolerância XTK customizada
        self.tolerance_input_rect = pygame.Rect(950, 175, 180, 46)
        self.btn_set_tolerance = pygame.Rect(1150, 175, 110, 46)
    ...
    def draw_data_box(self, surface, rect, title, value, color=WHITE):
        ...
        self.draw_text(surface, title, (rect.centerx, rect.y + 12), self.f_label, LIGHT_GRAY, align="center")
    ...
    def draw_flight_director(self, surface, nav_state, current_alt):
        ...
        # Oculta o FD se o sistema não estiver engajado, ou se estiver nas fases 1 a 5 (antes da curva de interceptação)
        if not is_active or (1 <= v_phase <= 5):
            return 
    ...
    def draw_navigation_panel(self, surface, data, nav_state, mouse_pos, active_input, course_text, tolerance_text):
        """
        ...
        # ── LINHA 2: Curso desejado e Tolerância ──────────────────────────────
        ...
        self.draw_text(surface, "Curso desejado (0-359):", (self.course_input_rect.x, self.course_input_rect.y - 20), self.f_label, LIGHT_GRAY)
        ...
        if not course_text and not is_course_active:
            self.draw_text(surface, "ex: 170", (self.course_input_rect.x + 12, self.course_input_rect.y + 11), self.f_label, GRAY)
        else:
            self.draw_text(surface, display_course, (self.course_input_rect.x + 12, self.course_input_rect.y + 8), self.f_btn, WHITE)
        ...
        self.draw_text(surface, "SET", self.btn_set_course.center, self.f_btn, WHITE, align="center")

        # ── LINHA 2: Tolerância XTK (direita) ────────────────────────────────
        ...
        self.draw_text(surface, "Tolerância XTK (NM):", (self.tolerance_input_rect.x, self.tolerance_input_rect.y - 20), self.f_label, LIGHT_GRAY)
        ...
        if not tolerance_text and not is_tolerance_active:
            self.draw_text(surface, "ex: 0.3", (self.tolerance_input_rect.x + 12, self.tolerance_input_rect.y + 11), self.f_label, GRAY)
        else:
            self.draw_text(surface, display_tolerance, (self.tolerance_input_rect.x + 12, self.tolerance_input_rect.y + 8), self.f_btn, WHITE)
        ...
        self.draw_text(surface, "SET", self.btn_set_tolerance.center, self.f_btn, WHITE, align="center")
        ...
        record_label = f"GRAVAR WYP ({wp_count}/{max_wps})"
        ...
        if is_active:
            ...
            if nav_mode == "route" and waypoints:
                ...
                self.draw_text(surface, "ALVO", (600, 325), self.f_label, LIGHT_GRAY)
                self.draw_text(surface, wp_name, (600, 347), self.f_data, MAGENTA)
            else:
                self.draw_text(surface, "CURSO TRAVADO", (600, 325), self.f_label, LIGHT_GRAY)
            ...
            if not (1 <= v_phase <= 5):
                self.draw_text(surface, "DESVIO LATERAL (XTK)", (920, 325), self.f_label, LIGHT_GRAY)
            ...
            if nav_mode == "route_finished":
                ...
                if not c_side:
                    ...
                    if v_phase == 1:
                        prl_hdg = (course + 180) % 360
                        self.draw_text(surface, f"CURVAR PARALELA -> {int(prl_hdg):03}°", (620, ai_card_y + 88), self.f_data, MAGENTA)
                    elif v_phase == 2 or v_phase == 3:
                        prl_hdg = (course + 180) % 360
                        self.draw_text(surface, f"PARALELA -> {int(prl_hdg):03}°", (620, ai_card_y + 88), self.f_data, BLUE)
            ...
            if nav_mode == "route_finished":
                ...
                if v_phase == 1:
                    if not c_side:
                        ...
                        explanation = f"SISTEMA: Circuito pela {lado} identificado. Siga a proa do Flight Director para curvar e estabelecer na perna paralela."
                elif v_phase == 2:
                    explanation = f"SISTEMA: Circuito pela {lado} identificado. Aeronave voando perna paralela. Mantenha proa até cruzar o través do WP1."
                elif v_phase == 3:
                    explanation = f"SISTEMA: Través do WP1 cruzado. Afastando no eixo para criar distância ideal de curva à {lado}..."
                elif v_phase == 4:
                    explanation = f"SISTEMA: Distância ideal atingida (>3.0 NM). Inicie curva BASE à {lado} para interceptação suave."
                elif v_phase == 5:
                    explanation = f"SISTEMA: Aeronave estabelecida na perna BASE. Siga a proa sugerida até a área de interceptação do curso."
                elif v_phase == 6:
                    explanation = f"SISTEMA: Aproximando-se da área de interceptação. Curvando para INBOUND dinâmico."
            else:
                if v_phase == 7:
                    ...
                    # Calcula direção da curva com base na proa atual
                    ...
                    # Mensagem adaptada ao modo rota vs manual
                    if nav_mode == "route" and waypoints:
                        wp_name = waypoints[active_wp_idx].get("name", f"WP{active_wp_idx+1}")
            ...
            if nav_state.get("system_on", False):
                ...
                self.draw_text(surface, "Clique em 'INICIAR MONITOR' para ligar os sistemas.", (panel_rect.centerx, 420), self.f_label, LIGHT_GRAY, align="center")
    ...
    def draw_vnav_panel(self, surface, data, nav_state, mouse_pos, active_input, vnav_alt_text, vnav_tol_text, vnav_cal_text):
        ...
        self.draw_text(surface, "ALTITUDE ATUAL", (panel_rect.centerx, 110), self.f_label, LIGHT_GRAY, align="center")
        ...
        self.draw_text(surface, "Calibrar ALT (FT):", (self.vnav_cal_input_rect.x, self.vnav_cal_input_rect.y - 20), self.f_small, LIGHT_GRAY)
        ...
        self.draw_text(surface, "Altitude Desejada (FT):", (self.vnav_alt_input_rect.x, self.vnav_alt_input_rect.y - 20), self.f_label, LIGHT_GRAY)
        ...
        self.draw_text(surface, "Variação Aceita (+/- FT):", (self.vnav_tol_input_rect.x, self.vnav_tol_input_rect.y - 20), self.f_label, LIGHT_GRAY)
        ...
        # Exibe status VNAV apenas fora das fases de vetoramento inicial (1 a 5)
        if target_alt is not None and not (1 <= v_phase_vnav <= 5):
    ...
    def draw_procedure_panel(self, surface, mouse_pos, selected_chart_name, available_charts, show_dropdown=False, nav_state=None, show_dir_dropdown=False):
        ...
        self.draw_text(surface, f"CARTA: {display_name}", (self.btn_cycle_chart.x + 10, self.btn_cycle_chart.y + 8), self.f_msg, WHITE)
...
def main():
    ...
    active_input = "none"       # Foco ativo: "course", "tolerance", "timer", "vnav_alt", "vnav_tol", "vnav_cal" ou "none"
    ...
    while running:
            ...
            if event.type == pygame.QUIT:
                ...
                if gui.btn_return_wp.collidepoint(mouse_pos):
                    ...
                    if len(wps) > 0 and data.get("connected"):
                        ...
                        first_wp = wps[0]
                        # A primeira perna até o IAF é um Direct-To da posição atual do avião
        ...
        if nav_state["active"] and data.get("connected"):
            ...
            if nav_state["navigation_mode"] == "route":
                ...
                if idx < len(wps):
                    ...
                    if v_phase > 0:
                        ...
                        # A IA não força um lado. Ela observa a manobra do piloto após a arremetida.
                        circuit_side = nav_state.get("circuit_side")
                        if not circuit_side:
                            # Se o piloto afastar lateralmente da rota em mais de 0.5 NM, a IA "trava" o lado.
                            if abs(xtk_nm) > 0.5:
                        ...
                        if not circuit_side:
                            ...
                            # PRIORIDADE 2: Voando sentido oposto ao curso (Paralela)
                    ...
                    WP_CAPTURE_RADIUS_NM = 0.15 if is_mapt else 0.3
                    ...
                    if dist_to_wp < WP_CAPTURE_RADIUS_NM or passed_abeam or turn_anticipation or passed_cpa:
                        ...
                        nav_state["vectoring_phase"] = 0 # RETORNA PARA RASTREAMENTO NORMAL (ALARMES LIGADOS)
                        next_idx = idx + 1
                        if next_idx < len(wps):
                            ...
                            if active_procedure:
                                # PROCEDIMENTO: A perna inicia no waypoint anterior (target_wp)
                                ...
                                # MANUAL: Nova perna = Direct-To da posição atual para próximo WP
                                nav_state["lat_orig"] = data["lat"]
                                nav_state["lon_orig"] = data["lon"]
                            ...
                            # Carrega altitude do próximo WP para o VNAV
            ...
            if v_phase_current == 0 or v_phase_current == 7:
                ...
                # PERNA PARALELA: tempo real igualzinho ao INBOUND
                # Usa calculate_intercept_heading com o recíproco do curso
                # XTK positivo (direita do eixo) = correto para circuito R, mas se aumentando demais,
                # o recíproco deve corrigir para esquerda (heading < reciprocal) - lógica natural de XTK
                course_true_val = nav_state.get("course_true", nav_state["course"])
                reciprocal_true = (course_true_val + 180) % 360
                # Mesma lógica do INBOUND mas para o recíproco: XTK positivo → heading abaixo do recíproco
                target_heading_true = NavMath.calculate_intercept_heading(reciprocal_true, xtk_nm)
                mag_var = data.get("mag_var", 0.0)
                nav_state["target_heading"] = int((target_heading_true - mag_var + 360) % 360)
            
            elif v_phase_current == 5:
                # PERNA BASE: tempo real igualzinho ao INBOUND
                # Usa calculate_intercept_heading com a proa da perna base
                course_true_val = nav_state.get("course_true", nav_state["course"])
                c_side = nav_state.get("circuit_side")
                if c_side == "L":
                ...
                target_heading_true = NavMath.calculate_intercept_heading(base_true, xtk_nm)
                mag_var = data.get("mag_var", 0.0)
                nav_state["target_heading"] = int((target_heading_true - mag_var + 360) % 360)
        ...
        if data.get("connected"):
            ...
            gui.draw_navigation_panel(canvas, data, nav_state, mouse_pos, active_input, course_text, tolerance_text)
